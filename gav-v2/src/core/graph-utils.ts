...
import { NODE_RADIUS } from "../canvas/hitTest";
...
/** Returns the visual radius of a node — fixed or degree-scaled depending on settings. */
export function getNodeRadius(
    nodeId: string,
    graph: GraphData,
    settings: Settings
): number {
    if (!settings.variableNodeSizes) return NODE_RADIUS;
    const degree = graph.edges.filter(
        (e) => e.source === nodeId || e.target === nodeId
    ).length;
    return Math.max(
        NODE_RADIUS,
        Math.min(NODE_RADIUS * 2, NODE_RADIUS + degree * 2)
    );
}

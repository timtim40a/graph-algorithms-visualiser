...
const GraphCanvas: React.FC<GraphCanvasProps> = (
    ...
    const handleNodeDelete = (idToDelete: string) => {
        ...
        const updateEdges = edges.filter(
            (edge) =>
                edge.targetID === idToDelete && edge.sourceID !== idToDelete
        )
    ...
    const handleCanvasClick = (e: React.MouseEvent<HTMLDivElement>) => {
        if (isEditModeOn) {
            ...
            addNode(id, id.slice(1), e.clientX, e.clientY, () =>
                handleNodeClick(id)
            )
    ...
    const nextFrame = () => {
        animationFrames[0][animationIndex].forEach(
            (animationMode, animationNodes) => {
                ...
                const edgeReverseID =
                    'e' + animationNodes[1] + animationNodes[0]
                const edgeToAnimate = edges.find(
                    (edge) => edge.id === edgeID || edge.id === edgeReverseID
                )

                if (edgeToAnimate) {
                    alterEdge(edgeToAnimate.id, {
                        activeAnimation: animationMode,
                    })
                }
        ...
        const animateFrame = async () => {
            animationFrames[0][animationIndex].forEach(
                (animationMode, animationNodes) => {
                    ...
                    const edgeToAnimate = edges.find(
                        (edge) =>
                            edge.id === edgeID || edge.id === edgeReverseID
                    )
    ...
    useEffect(() => {
        console.log(
            `Node deletion mode is now: ${isDeletingNode ? 'ON' : 'OFF'}`
        )
        /*setMessage([
        ...
        <>
            ...
            >
                ...
                <div className="left-sidebar">
                    ...
                    {isEditModeOn ? (
                        <>
                            ...
                            <br></br>
                            <button onClick={(e) => tenXtenGrid(e)}>
                                Create 10x10 grid
                            </button>
                            <br></br>
                        </>
                ...
                >
                    ...
                    {isAnimationOn && searchOrder?.heuristics
                        ? heuristicsEdges()
                        : null}
                </svg>
                {nodes.map((node) => (
